...
const source = await readFile(resolve(process.cwd(), "contracts/BaseHabitOrbit.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "BaseHabitOrbit.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["BaseHabitOrbit.sol"]?.BaseHabitOrbit;
...
console.log(`Deploying BaseHabitOrbit from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_HABIT_ORBIT_CONTRACT_ADDRESS to this contract address in Vercel.");
